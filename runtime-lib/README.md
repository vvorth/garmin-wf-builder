...
| `WfbHands.mc` | analog hands (plan 04): `hourAngle`/`minuteAngle`/`secondAngle` from a `System.ClockTime`, plus `fillRotated`/`drawLineRotated`/`fillCircleRotated`/`drawCircleRotated` — each rotates a hand's build-time-resolved geometry by one `sin`/`cos` pair and draws it. `sin`/`cos` are typed `Decimal`, not `Float` — `Math.sin`/`Math.cos` are declared to return `Float or Double`, and strict typing fails a narrower parameter |
