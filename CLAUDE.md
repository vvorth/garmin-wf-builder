...
| proposals written but not built | `docs/plans/`: **18** (bug fixes from the 2026-09-24 review: built except two small items in its §2, one awaiting user decision) and **19** (architecture proposals, awaiting user decision; §3 is the combined 18+19 order). Built plans are deleted — see `docs/CLAUDE.md` |
