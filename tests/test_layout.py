...
def test_widest_text_accounts_for_a_longer_fallback(write_design, bag, db):
    ...
    value (see `wfb.kinds.text.TextKind.emit_draw`), so a font baked from the
...
def test_geometry_left_matches_todays_layout():
    ...
    exactly the box `wfb.emit.monkeyc.complication_slot.emit_complication_slot`'s fast path
    has always drawn."""
