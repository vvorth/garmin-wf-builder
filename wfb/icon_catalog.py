...
**One catalogue, no exceptions.** A weather condition's glyph is exactly as
much an icon as a heart or a flame is, so it lives in the same
:data:`CATALOG` as everything else rather than a separate, curated table --
a second table would just be a second place to keep in sync. The device
side follows the same rule: ``source/IconGlyphs.mc`` (generated by
:mod:`wfb.emit.monkeyc` from this same file, not hand-written) is the one
generic name -> glyph resolver, so ``runtime-lib/WfbWeather.mc`` only ever
needs to answer "which *name*", in Number -> String terms -- exactly
mirroring how :func:`wfb.icons.weather_icon_for_condition` separates "which
name" (:data:`wfb.icons.GARMIN_WEATHER_CONDITION_ICON`) from "which glyph"
(this file, via ``CATALOG[name].codepoint``).
...
pasted as a raw character, and *only* that way. The actual glyph is invisible
in most editors and terminals (that is the whole reason it needs an icon
font), so a literal escape is what stays readable, greppable, and safe to
move between files without silent corruption -- a raw character that fails
to paste correctly still parses as valid Python (an empty string), so the
mistake only surfaces later as a blank tile or a missing-glyph assertion.
Every entry also carries the actual
