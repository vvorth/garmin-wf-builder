...
needs to answer "which *name*", in Number -> String terms -- "which name"
is :data:`wfb.icons.GARMIN_WEATHER_CONDITION_ICON`, "which glyph" is this
file, via ``CATALOG[name].codepoint``.
...
CATALOG: dict[str, Icon] = {
    ...
    for icon in [
        ...
        # have no `_night` entry: an author uses the day glyph there.
