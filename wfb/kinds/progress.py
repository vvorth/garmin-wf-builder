...
def resolve(r, element: Progress, parent: Box, depth: int) -> Placed:
...
def draw_preview(renderer, placed: PlacedProgress) -> None:
    element = placed.element
    value = expr.evaluate(element.value.ast, renderer.values) if element.value.ast else None
    maximum = expr.evaluate(element.maximum.ast, renderer.values) if element.maximum.ast else None
...
def emit_draw(w: Writer, resolved, placed: PlacedProgress, guards: list[str], plan,
...
def layout_constants(prefix: str, placed: PlacedProgress) -> "layout_constants_mod.Constants":
