...
def resolve(r, element: ComplicationSlot, parent: Box, depth: int) -> Placed:
...
def draw_preview(renderer, placed: PlacedComplicationSlot) -> None:
...
def emit_draw(w: Writer, resolved, placed: PlacedComplicationSlot, value_guards, plan,
              aod: AodStyle = NO_AOD) -> None:
    complication_slot_mod._emit_complication_slot(w, resolved, placed, plan.device_guards, aod)


def layout_constants(prefix: str, placed: PlacedComplicationSlot) -> "layout_constants_mod.Constants":
...
def glyph_needs(element: ComplicationSlot, face, bucket) -> None:
    ...
    # (`_complication_slot_widest`, same reason).
