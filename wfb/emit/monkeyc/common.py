...
# -- `aod: {jitter: ...}` (plan 14 slice 5, §5.2) ---------------------------


def _jitter_field(n: int, axis: str) -> str:
    """The view field one distinct jitter magnitude's own per-frame offset
    lives in -- ``_aodDxN<n>``/``_aodDyN<n>``, one pair *per distinct `n`
    actually used anywhere in the design* (not one pair per jittered group):
    the offset is a pure function of `(minute of day, n)`
    (`wfb.aod_jitter.offset`), so two groups that happen to share the same
    `n` always move identically, and there is nothing to gain from a second,
    independently-computed pair for the second one. Suffixed even when the
    design only ever uses one `n` -- simpler and more predictable than
    special-casing "exactly one magnitude in use" for a cosmetically shorter
    field name.
    """
    assert axis in ("x", "y")
    return f"_aodD{axis}N{n}"


def _jitter_terms(placed, aod: bool) -> tuple[str, str]:
    """``(dx_term, dy_term)`` to append, verbatim, to this element's own X
    and Y `Layout` constants -- `""` for both when this build never emits
    AOD code at all (``aod`` -- some target is AMOLED, `wfb.availability.
    Guards.amoled_target`, the same flag every other `_aod_*` helper in
    this module takes) or when this element does not resolve into a
    jittered AOD scope (`Element.aod_jitter is None`, or the element does
    not draw in AOD at all, `Element.aod is None`), else
    `" + _aodDxN<n>"`/`" + _aodDyN<n>"`.

    **`aod` is not optional to check here.** `Element.aod`/`.aod_jitter`
    are resolved once per *face* (`Builder._resolve_aod`/`_resolve_aod_
    jitter`), before any device is chosen, so they are set the same way
    whether or not the *current build's* targets include an AMOLED device
    -- unlike `emit_view`'s own `jitter_ns` (`_aod_jitter_ns`, already
    gated on `aod`), which decides whether the `_aodDxN<n>`/`_aodDyN<n>`
    *fields themselves* get declared at all. Skipping this check would
    reference an undeclared field on an all-MIP build -- caught by
    `tests/test_aod_jitter.py::
    test_an_all_mip_build_is_byte_identical_with_jitter_declared`, which
    failed with exactly that shape before this parameter existed.

    **Otherwise unconditional, never an `_aod ? ... : ...` ternary** --
    unlike every other override in this module. The view's own
    `_aodDxN<n>`/`_aodDyN<n>` fields are `0` at every moment except while
    the AOD branch that computed them for *this* frame is the one
    currently drawing (`emit_view` resets them to `0` in `onExitSleep`, and
    only `_emit_aod_body` ever assigns them anything else), so appending
    them unconditionally, at every coordinate a jittered element's shared
    draw method uses -- the same method the *awake* branch also calls -- is
    behaviourally identical to a ternary while being one fewer branch (plan
    14 §5.2: "an unconditional `+ _aodDx` ... is acceptable if it's smaller
    than branching").
    """
    element = placed.element
    if not aod or element.aod is None or element.aod_jitter is None:
        return "", ""
    n = element.aod_jitter
    return f" + {_jitter_field(n, 'x')}", f" + {_jitter_field(n, 'y')}"


def _aod_jitter_ns(resolved: "ResolvedFace") -> tuple[int, ...]:
    """Every distinct jitter magnitude actually in use by an AOD-shown
    element anywhere in this build, sorted -- `()` when the design declares
    no `jitter:` at all, or declares one that no AOD-shown element ever
    resolves to (a `jitter:` written only on a face/group with nothing
    drawn in AOD beneath it). `emit_view` uses this to decide, once, which
    `_aodDxN<n>`/`_aodDyN<n>` field pairs -- and which `WfbJitter` calls
    computing them -- this particular build needs to emit at all, so a
    design that never uses `jitter:` (or an all-MIP build, where `aod`
    itself is already `False`) stays byte-identical to before this feature
    existed.
    """
    return tuple(sorted({
        placed.element.aod_jitter
        for placed in resolved.items
        if placed.kind != "group" and placed.element.aod is not None
        and placed.element.aod_jitter is not None
    }))
