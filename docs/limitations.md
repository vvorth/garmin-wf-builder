...
| **Text overflow** | Exact for a baked custom font (real glyph advances from the TrueType source). A system font (`FONT_TINY` and so on) is **usually an estimate** — Garmin publishes each `FONT_*` symbol's pixel *height* per device and language, but not its per-glyph advances. Since plan 09 (2026-09-18) the estimate measures against the device's own real typeface when `wfb/fonts/fetch_system.py` can locate one — the user's own licensed Garmin font files (`vendor/fonts/`) rank first, then a pinned free stand-in (`exact`/`family`/`substitute` match, `docs/research/10-system-fonts.md`'s mapping) — scaled to the device's own published metrics (`em_px`/`ascent_px`/`height_px` from the installed device's `simulator.json` when present, else derived from the located TTF's own `hhea` table against the published line height; `wfb/fonts/fallback.py`). Only when nothing can be located at all does it fall back to Pillow's bundled default face, and only when even that is unavailable to a flat 0.55 em/character coefficient. Every system-font measurement is labelled `(estimated)` in the generated code regardless, and a `substitute`/`none` match is still a different family's shape, not the real device font's. **Not an estimate on the 8 of 13 installed devices that are bitmap-font-only** (`fenix6`, `fenix6xpro`, `fenix7pro`, `fenix7x`, `fenix7xpro`, `fenix7xpronowifi`, `fr245`, `fr255`) when `vendor/fonts/` is present: every `FONT_*` symbol there resolves to a Garmin `.cft` bitmap font, decoded pixel-for-pixel (`wfb/fonts/cft.py`, plan 10 Step A) and used for both measurement and the preview's own ink (plan 10 Step B) — no TrueType stand-in involved at all. Three questions about that decode stay UNVERIFIED until the user's simulator results come back (`docs/research/10-system-fonts.md` §10.6/§10.7): whether `Graphics.getFontHeight` on such a device reports the `.cft`'s own `height` or `height - 1`; whether the simulator quantises the antialias blend to the 64-colour MIP palette; and what glyph the device itself draws for a character absent from the font's cmap (this project assumes glyph 0, the "missing" box). |
