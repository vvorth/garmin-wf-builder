...
| **Text overflow** | Exact when the font's real metrics are available (a baked custom font, or a system font with published per-device pixel metrics). Otherwise the extent is **estimated at 0.55 em per character**, and the diagnostic says so. |
