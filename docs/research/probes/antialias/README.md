...
call to `wfb.kinds.shape.emit_draw`/`wfb.kinds.progress.emit_draw`, and every one of them can `return`
...
inspection that no guard sits *inside* `wfb.kinds.shape.emit_draw`/`wfb.kinds.progress.emit_draw`
