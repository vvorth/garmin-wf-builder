...
(It would still typecheck without the guard: `../device-symbol-gate/` shows a
symbol absent from `fr955.api.debug.xml` compiling warning-free for `fr955`.
The runtime `has` guard is what keeps it safe.)
...
call to `wfb.kinds.shape.ShapeKind.emit_draw`/`wfb.kinds.progress.ProgressKind.emit_draw`, and every one of them can `return`
...
inspection that no guard sits *inside* `wfb.kinds.shape.ShapeKind.emit_draw`/`wfb.kinds.progress.ProgressKind.emit_draw`
