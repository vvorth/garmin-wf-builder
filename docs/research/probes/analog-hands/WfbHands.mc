//! Probe: the candidate runtime barrel for analog hands.
//!
...
//! per-device Layout constant; the only arithmetic left for the watch is the
//! rotation, because the angle is the time.
//!
...
module WfbHands {
    ...
    //! A polygon part: rotate every vertex about the axis, then fill.
    ...
    //! A circle part: only its centre moves; a circle is its own rotation.
