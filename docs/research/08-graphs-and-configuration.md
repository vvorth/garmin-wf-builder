...
### `getComplicationDrawable` drives the editor's highlight

It lets the editor animate a preview of the selected slot, and it is required
for that highlight: the SDK sample's own comment says the view must hide the
slot while the system pulses it, or it is drawn twice. The generated view draws
straight to the `Dc`, so the face emits a `Drawable` subclass that delegates
back to the view's own per-slot draw method, keeping one implementation of what
a slot looks like. `docs/research/probes/config-axes/` measured it on all three
targets at **+134 B data, +414 B code**. See
...
> (`layouts: <name>: {static, elements}`), not the element-level `styles:
> [minimal, rich]` membership list floated here, which this project calls
> "form B" and does not build. The "three areas × four styles is four
> entries, not sixty-four" arithmetic is exactly what shipped:
> `config: style:` is an author-written, ordered list of entries, each
> naming a layout, a `color_scheme:`, or both — no generated product. Hold-
> to-cycle stayed declined. `select: fixed`/per-device `overrides:` remain
> unbuilt, unrelated to this feature.
