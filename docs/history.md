...
This file is the chronological session-by-session narrative for
`garmin-wf-builder`'s Phase 3 ("breadth"): every feature built, every bug
found and fixed, every measurement taken, and every wrong turn and its
correction, in the order it happened.

**Why it lives here and not in `CLAUDE.md`:** by the time this file was split
out, this narrative was 131 KB — 83% of `CLAUDE.md` — and was being loaded in
full into every session's context for a payoff of a handful of durable facts.
`CLAUDE.md` now carries only what a session needs on turn one: the platform
constraints, the decisions, the current pipeline, a live Phase 3 roadmap, and
a short list of hard-won technical facts. Everything else — the *how* and
*why* behind each of those, and everything superseded along the way — is here.

**This is a relocation, not a deletion**, and that distinction matters to how
you read it. This project's house style has always been to leave a superseded
account in place and add a correction next to it, rather than delete it,
because the original reasoning is instructive — a wrong turn taken for
sound-seeming reasons is worth more to a future session than a clean
retelling that hides the reasons the mistake looked right at the time. That is
why this file is long, chronological, and unedited rather than a cleaned-up
changelog: it **is** the record that principle was already producing, just
moved out of the file that gets read on turn one. A few features described
below **no longer exist** — `type: carousel` and the `on_tap:` key both
shipped and were later removed outright, and the refresh-tier/`slow`-tier
mechanism shipped and was then deleted. Each such passage is flagged
**Superseded** at or near the top of its own section, since — unlike a reader
going straight through `CLAUDE.md` — you may have arrived at any one heading
here directly, without the context a linear read would have given you.

**Where new sessions go:** append new session accounts to the end of this
file, each under its own heading, in chronological order — do not insert them
elsewhere or renumber what is already here. Add an entry to the table of
contents below. Then lift into `CLAUDE.md` only what changed about what a
future session needs to know on turn one — an updated Status line, a new
platform constraint, a roadmap checklist item, a genuinely reusable hard-won
fact — following the same judgement calls recorded in `CLAUDE.md`'s own
opening note about this split. Do not re-inline the narrative back into
`CLAUDE.md`; that is the mistake this split exists to prevent.

`CLAUDE.md` is the durable half of this same record — read it first for
current status, platform constraints, decisions, and the live Phase 3
roadmap; come here for the reasoning and the full account of how it got
there.

## Contents

- [Authoring ergonomics research, and the skill](#authoring-ergonomics-research-and-the-skill)
- [Phase 3 — breadth](#phase-3--breadth)
  - [The six-feature session: new shapes, font `size:` as a `Length`, `monospace:`+`align:`, `visible:`, the `elements:` mapping form, and `static:`](#the-six-feature-session-new-shapes-font-size-as-a-length-monospacealign-visible-the-elements-mapping-form-and-static)
  - [Git note: commit 614d100 and the big-clock-3 assets scare](#git-note-commit-614d100-and-the-big-clock-3-assets-scare)
  - [`examples/dashboard/`: can the schema express Dashboard?](#examplesdashboard-can-the-schema-express-dashboard)
  - [Icon misuse in the dashboard example, and the move to a vendored icon font](#icon-misuse-in-the-dashboard-example-and-the-move-to-a-vendored-icon-font)
  - [The raw-glyph ceiling: Material Design Icons and a resource-compiler `filter` bug](#the-raw-glyph-ceiling-material-design-icons-and-a-resource-compiler-filter-bug)
  - [Icon size regression: nf-md padding and `bake_size`](#icon-size-regression-nf-md-padding-and-bake_size)
  - [Dynamic weather-condition icons, and the first real `slow`-tier source](#dynamic-weather-condition-icons-and-the-first-real-slow-tier-source)
  - [Weather-icon catalogue unification, part 1 (the Python side)](#weather-icon-catalogue-unification-part-1-the-python-side)
  - [Weather-icon catalogue unification, part 2 (the Monkey C side)](#weather-icon-catalogue-unification-part-2-the-monkey-c-side)
  - [The data-source catalogue grows from 28 to 45 entries](#the-data-source-catalogue-grows-from-28-to-45-entries)
  - [Building the `event` refresh tier for Complications (later deleted outright)](#building-the-event-refresh-tier-for-complications-later-deleted-outright--see-below)
  - [`wfb` becomes self-describing: `wfb help`](#wfb-becomes-self-describing-wfb-help)
  - [Review session: seven logical-error findings, and two linter-credibility bugs](#review-session-seven-logical-error-findings-and-two-linter-credibility-bugs)
  - [`glyph:`, `on_tap:` (since renamed `on_hold:`), and a rewritten glyph rasteriser](#glyph-on_tap-since-renamed-on_hold-and-a-rewritten-glyph-rasteriser)
  - [The carousel-interaction research, and `type: carousel` ships (later removed outright)](#the-carousel-interaction-research-and-type-carousel-ships-later-removed-outright--see-below)
  - [The refresh-tier concept is deleted outright; all 42 complication types become data sources](#the-refresh-tier-concept-is-deleted-outright-all-42-complication-types-become-data-sources)
  - [Anti-aliasing: `antialias:` spans two unrelated SDK mechanisms](#anti-aliasing-antialias-spans-two-unrelated-sdk-mechanisms)
  - [Static draw order becomes an automatic rule, not a build error](#static-draw-order-becomes-an-automatic-rule-not-a-build-error)
  - [`type: graph`, and `config:`'s two colour axes](#type-graph-and-configs-two-colour-axes)
  - [Research only: could a `data_library` and a colour-scheme axis be configured on the watch?](#research-only-could-a-data_library-and-a-colour-scheme-axis-be-configured-on-the-watch)
  - [Building research 09's recommendation: palette labels, `color_scheme:`, and the Data axis](#building-research-09s-recommendation-palette-labels-color_scheme-and-the-data-axis)
  - [`on_hold:` on a `complication_slot`, and the native editor's animated highlight](#on_hold-on-a-complication_slot-and-the-native-editors-animated-highlight)
...
## Authoring ergonomics research, and the skill
...
> The numbered list immediately below is the **original** Phase 3 roadmap,
> written early in Phase 3 and left as written — it is superseded by
> `CLAUDE.md`'s "Phase 3 roadmap", which is kept current. Read this list as
> a historical snapshot of the plan, not as today's status; in particular,
> item 5's `type: carousel` discussion describes a feature that **shipped
> and was later removed outright** — see "`type: carousel` is deleted
> entirely" further down this file.
...
### The six-feature session: new shapes, font `size:` as a `Length`, `monospace:`+`align:`, `visible:`, the `elements:` mapping form, and `static:`
...
### Git note: commit 614d100 and the big-clock-3 assets scare
...
### `examples/dashboard/`: can the schema express Dashboard?
...
### Icon misuse in the dashboard example, and the move to a vendored icon font
...
### The raw-glyph ceiling: Material Design Icons and a resource-compiler `filter` bug
...
### Icon size regression: nf-md padding and `bake_size`
...
### Dynamic weather-condition icons, and the first real `slow`-tier source
...
### Weather-icon catalogue unification, part 1 (the Python side)
...
### Weather-icon catalogue unification, part 2 (the Monkey C side)
...
### The data-source catalogue grows from 28 to 45 entries
...
### Building the `event` refresh tier for Complications (later deleted outright — see below)
...
### `wfb` becomes self-describing: `wfb help`
...
### Review session: seven logical-error findings, and two linter-credibility bugs
...
### `glyph:`, `on_tap:` (since renamed `on_hold:`), and a rewritten glyph rasteriser
...
### The carousel-interaction research, and `type: carousel` ships (later removed outright — see below)
...
### The refresh-tier concept is deleted outright; all 42 complication types become data sources
...
### Anti-aliasing: `antialias:` spans two unrelated SDK mechanisms
...
### Static draw order becomes an automatic rule, not a build error
...
### `type: graph`, and `config:`'s two colour axes
...
### Research only: could a `data_library` and a colour-scheme axis be configured on the watch?
...
### Building research 09's recommendation: palette labels, `color_scheme:`, and the Data axis
...
### `on_hold:` on a `complication_slot`, and the native editor's animated highlight
...
### Review session: seven more logical-error findings, all fixed
...
### `type: carousel` is deleted entirely
...
### The bare-number font `size:` + `scale:` spelling is deleted outright
